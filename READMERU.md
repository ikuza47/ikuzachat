...
- **Правильная замена эмодзи** которая не ломает текстовые последовательности как "((( "
- **Цветные упоминания пользователей** с персональными цветами
- **Настройка отображения времени** с поддержкой часовых поясов
...
---
## Модули
- **osu! модуль**: Включает поддержку отображения информации о картах osu! по ссылке (требуется API ключ)
- **Модуль блокировки ботов**: Скрывать сообщения от ботов (список загружается с GitHub)
...
- [X] Правильная замена эмодзи (предотвращение поломки текста как "((( ")
- [ ] Поддержка твич-эмодзи (через теги)
- [ ] Пресеты
