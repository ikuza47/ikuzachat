                    ...
                    try {
                        ...
                        if (window.botModule && typeof window.botModule.isUserBot === 'function') {
                            if (window.botModule.isUserBot(username)) {
                                console.log(`🚫 Сообщение от бота ${username} проигнорировано`);
                        ...
                        if (hasHtmlTags) {
                            console.log('💬 Сообщение содержит HTML-теги, экранируем только теги');

                            // Если есть потенциальные сущности, нужно быть осторожным
...
async function startInitialization() {
    ...
    // Загружаем список ботов, если модуль включен
    if (window.botModule && typeof window.botModule.loadBotList === 'function') {
        console.log('🔄 Загрузка списка ботов...');
        await window.botModule.loadBotList();
    }
}

// Запускаем процесс инициализации
startInitialization().then(() => {
    // Если включен тестовый режим - запускаем его вместо подключения к чату
    if (window.testMode) {
        console.log('🧪 Тестовый режим включен, запуск генерации сообщений');
        if (window.testModeModule && typeof window.testModeModule.start === 'function') {
            window.testModeModule.start();
        }
    } else {
        // В противном случае подключаемся к чату
        setTimeout(connectToChat, 1000);
        startReadyCheck();
    }
}).catch(error => {
    ...
    // Если тестовый режим включен, запускаем его, иначе подключаемся к чату
    if (window.testMode) {
        console.log('🧪 Тестовый режим включен, запуск генерации сообщений');
        if (window.testModeModule && typeof window.testModeModule.start === 'function') {
            window.testModeModule.start();
